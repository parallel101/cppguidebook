...
- `define` 表示这是一个函数的定义。
- `dso_local` 是函数 `main` 的修饰符，意味着编译器可以假设标记该函数或变量被解析为同一链接单元内的符号，dso代表动态共享对象（dynamic shared object），可以先无视。
...
如果函数返回类型不为 `void`，则可以指定一个返回值（常数或寄存器的形式）。例如这里指定了 `i32 %5` 就表示函数会返回寄存器 `%5` 的值。
